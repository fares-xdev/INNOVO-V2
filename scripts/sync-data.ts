...
async function syncSocialLinks() {
  ...
  try {
    ...
    writeJSON('social-links.json', data);
  } catch (error) {
    console.warn("  ⚠️  Social Links API failed, using static fallback:", (error as Error).message);
    const fallbackData = [
      {
        id: 1,
        title: { rendered: "Facebook" },
        link: "https://www.facebook.com/Innovo.eg",
        slug: "facebook"
      },
      {
        id: 2,
        title: { rendered: "Instagram" },
        link: "https://www.instagram.com/innovo_officefurniture/",
        slug: "instagram"
      },
      {
        id: 3,
        title: { rendered: "LinkedIn" },
        link: "https://www.linkedin.com/company/innovo-office-furniture",
        slug: "linkedin"
      }
    ];
    writeJSON('social-links.json', fallbackData);
  }
}
