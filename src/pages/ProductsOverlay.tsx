...
const ProductsOverlay = () => {
  ...
  const brandCountsQuery = useQuery({
    queryKey: ["brand-counts-overlay", activeTypeSlugs.join(","), activeCatSlugs.join(","), searchQuery],
    enabled: !!filters,
    queryFn: () => {
  ...
  const categoryCountsQuery = useQuery({
    queryKey: ["category-counts-overlay", activeBrandSlugs.join(","), activeTypeSlugs.join(","), searchQuery],
    enabled: !!filters,
    queryFn: () => {
      const params: Record<string, string | number | string[]> = {
  ...
  const typeCountsQuery = useQuery({
    queryKey: ["type-counts-overlay", activeBrandSlugs.join(","), activeCatSlugs.join(","), searchQuery],
    enabled: !!filters,
    queryFn: () => {
      const params: Record<string, string | number | string[]> = {
  ...
  const { 
    ...
    queryKey: ["products-overlay", currentPage, activeTypeSlugs.join(","), activeBrandSlugs.join(","), activeCatSlugs.join(","), searchQuery, itemsPerPage],
    enabled: !!filters,
    queryFn: () => {
      const params: Record<string, string | number | string[]> = {
        page: currentPage,
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <div
        className={`fixed inset-y-0 left-0 w-80 bg-background z-[70] transition-transform duration-300 shadow-xl flex flex-col ${
                    ...
                    {hasChildren && isExpanded && (
                      <div className="pl-6 space-y-1 border-l border-gray-100 ml-2">
                        {parent.children.map((child: any) => (
                          ...
                          >
                            <Checkbox 
                              id={`type-overlay-child-${child.id}`} 
                              checked={activeTypeSlugs.includes(child.slug)}
                              onCheckedChange={() => {}} 
                              className="border-gray-300 data-[state=checked]:bg-primary data-[state=checked]:border-primary"
                            />
                            <Label 
                              htmlFor={`type-overlay-child-${child.id}`}
                              className="text-sm font-medium leading-none cursor-pointer group-hover:text-primary transition-colors flex-1"
                            >
            ...
            <div className="space-y-1">
              {categories.map((cat) => (
                <div key={cat.id} className="flex items-center space-x-3 group cursor-pointer p-1.5 rounded-md hover:bg-gray-50 transition-all" onClick={() => toggleFilter("filter_cat", cat.slug)}>
                  <Checkbox 
                    id={`cat-overlay-${cat.id}`} 
                    checked={activeCatSlugs.includes(cat.slug)}
                    onCheckedChange={() => {}} 
                    className="border-gray-300 data-[state=checked]:bg-primary data-[state=checked]:border-primary"
                  />
                  <Label 
                    htmlFor={`cat-overlay-${cat.id}`}
                    className="text-sm font-medium leading-none cursor-pointer group-hover:text-primary transition-colors flex-1"
                  >
            ...
            <div className="space-y-1">
              {brands.map((brand) => (
                <div key={brand.id} className="flex items-center space-x-3 group cursor-pointer p-1.5 rounded-md hover:bg-gray-50 transition-all" onClick={() => toggleFilter("filter_brand", brand.slug)}>
                  <Checkbox 
                    id={`brand-overlay-${brand.id}`} 
                    checked={activeBrandSlugs.includes(brand.slug)}
                    onCheckedChange={() => {}} 
                    className="border-gray-300 data-[state=checked]:bg-primary data-[state=checked]:border-primary"
                  />
                  <Label 
                    htmlFor={`brand-overlay-${brand.id}`}
                    className="text-sm font-medium leading-none cursor-pointer group-hover:text-primary transition-colors flex-1"
                  >
