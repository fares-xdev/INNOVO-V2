...
class HeroMeta {
	...
	public function render_hero_meta_box( $post ) {
		...
		<table class="form-table">
			...
			<tr>
				...
				<td><input type="number" name="slide_order" id="slide_order" value="<?php echo esc_attr($order); ?>" class="small-text"></td>
			</tr>
		</table>
		<?php
	}
	...
	public function save_metadata( $post_id ) {
		...
		if ( isset( $_POST['hero_description'] ) ) {
			update_post_meta( $post_id, self::META_DESCRIPTION, sanitize_textarea_field( $_POST['hero_description'] ) );
		}

		if ( isset( $_POST['slide_order'] ) ) {
			update_post_meta( $post_id, self::META_ORDER, intval( $_POST['slide_order'] ) );
		}
	}
